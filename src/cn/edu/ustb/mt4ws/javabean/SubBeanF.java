...
public class SubBeanF {
	...
	public SubBeanF(int id, int mrID, int sID, String a, String b) {
		super();
		this.id = id;
		this.mrID = mrID;
		this.sID = sID;
	 ...
	 * @return
	 */
	public static SubBeanF generateFollowTC(SubBean subBean,
			WsdlOperationFormat opFormat, MetamorphicRelation mr) {

		SubBeanF tcSubF = new SubBeanF();

		XmlTestCase xmlTC_S = getXmlTC(subBean, opFormat);
		...
		inputFollow = mod.modifyTransfer(mr.getRelationOfInput(), xmlTC_S
				.getInput());
		tcSubF.setsID(subBean.getId());// 设置衍生用例中的sID为原始测试用例的ID
		tcSubF.setMrID(subBean.getMrID());
	 ...
	 * @return
	 */
	public static XmlTestCase getXmlTC(SubBean subBean,
